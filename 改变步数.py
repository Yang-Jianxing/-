...
LexinSport:    def __init__(self, username, password, step):        self.username = username        self.password = password        self.step = step    # 登录    def login(self):        url = 'https://sports.lifesense.com/sessions_service/login?systemType=2&version=4.6.7'
...
if __name__ == "__main__":
    ...
    while 1:
        while fail_num > 0:
            try:                # 修改步数结果                result = LexinSport('13919589348', '12580zxcvbnm', 50000).change_step()
                print(result)
                break
